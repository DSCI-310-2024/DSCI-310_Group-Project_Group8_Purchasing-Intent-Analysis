...
@click.command()
@click.argument('features_input_path', type=str)
@click.argument('targets_input_path', type=str)
...
def create_train_test_split(features_input_path, targets_input_path, x_train_output_path, x_test_output_path, y_train_output_path, y_test_output_path, test_size, random_state):
    """
    ...
    X = pd.read_csv(features_input_path)
    y = pd.read_csv(targets_input_path)
